...
$sql = "SELECT b.booking_id, b.total_amount, b.total_seats, b.booking_date, b.status, 
               s.show_date, s.start_time, m.title, h.hall_name, u.full_name, u.email
        FROM bookings b
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($stmt->execute()) {
        ...
        $sqlRefresh = "SELECT b.booking_id, b.total_amount, b.total_seats, b.booking_date, b.status, 
                       s.show_date, s.start_time, m.title, h.hall_name, u.full_name, u.email
                FROM bookings b
                JOIN showtimes s ON b.showtime_id = s.showtime_id
                JOIN movies m ON s.movie_id = m.movie_id
        ...
        $stmtRefresh->bind_param('i', $booking_id);
        $stmtRefresh->execute();
        $resultRefresh = $stmtRefresh->get_result();
        $booking = $resultRefresh->fetch_assoc();
    } else {
        $error = "Failed to confirm booking: " . $conn->error;
    }
...
<section class="admin-confirm-booking">
    ...
    <form action="admin-confirm-booking.php?id=<?php echo $booking['booking_id']; ?>" method="post">
        <div class="form-actions">
            <button type="submit" class="btn confirm-btn">Confirm Booking</button>
            <a href="admin-manage-bookings.php" class="btn cancel-btn">Cancel</a>
        </div>
    </form>
</section>
